...
new class extends Component {
    ...
    public function mount($id)
    {
        ...
        $this->no_ketetapan    = $lokasi->no_ketetapan;
        $this->koordinat       = $lokasi->kord_lat . ', ' . $lokasi->kord_long;
        $this->status          = $lokasi->status;
        $this->tgl_registrasi  = $lokasi->tgl_registrasi;
        $this->area_id         = $lokasi->area_id;
        $this->kelurahan_id    = $lokasi->kelurahan_id;
        $this->korlap_id       = $lokasi->korlap_id;
        ...
        $this->tgl_ketetapan   = $lokasi->tgl_ketetapan;
        $this->is_jukir        = $lokasi->is_jukir;
        ...
        $this->oldGambar       = $lokasi->gambar;

        $this->areas = Area::all();
        $this->korlaps = Korlap::all();
        $this->kelurahans = Kelurahan::where('area_id', $this->area_id)->get();
    }
    ...
    public function updatedAreaId($value)
    {
        ...
        if(!in_array($this->kelurahan_id, $this->kelurahans->pluck('id')->toArray())) {
            $this->kelurahan_id = '';
        }
    }    
    ...
    public function save()
    {
        $this->validate([
            ...
            'slug'            => 'required|string|max:255|unique:lokasis,slug,' . $this->lokasiId,
        ...
        if ($this->koordinat) {
            $parts = preg_split('/[,\s]+/', trim($this->koordinat));
            $this->kord_lat = $parts[0] ?? null;
            $this->kord_long = $parts[1] ?? null;
        } else {
            $this->kord_lat = null;
            $this->kord_long = null;
        }
        ...
        if ($this->gambar) {
            ...
            $data['gambar'] = $this->gambar->store('lokasis', 'public');
        }

        $lokasi->update($data);

        session()->flash('success', 'Titik Parkir berhasil diperbarui.');
        return redirect()->route('lokasi.index');
    }

    public function render()
    {
        return $this->view()->title('Edit Titik Parkir');
    }
};
?>
                ...
                <div class="card">
                        ...
                        <div class="row">
                            <div class="col-md-12 mb-3">
                                ...
                                <div class="mt-2 text-center">
                                    ...
                                    @elseif ($oldGambar)
                                        <img src="{{ asset('storage/' . $oldGambar) }}" class="img-thumbnail" style="height: 200px;">
                                    @endif
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="card-footer text-end">
                        ...
                        <button type="submit" class="btn btn-primary">
                            <span wire:loading.remove wire:target="save">Update Titik Parkir</span>
