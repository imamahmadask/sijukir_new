...
new class extends Component {
    ...
    #[On('open-edit-berlangganan')]
    public function openModal($id)
    {
        $this->parkir_id = $id;
        $parkir = ParkirBerlangganan::findOrFail($id);
        
        $this->nomor = $parkir->nomor;
        $this->jenis = $parkir->jenis;
        $this->status = $parkir->status;
        $this->nama = $parkir->nama ?: $parkir->nama_pemilik;
        $this->no_pol = $parkir->no_pol;
        $this->alamat = $parkir->alamat;
        $this->jumlah = $parkir->jumlah;
        $this->masa_berlaku = $parkir->masa_berlaku ?? '6 Bulan';
        $this->awal_berlaku = $parkir->awal_berlaku;
        $this->akhir_berlaku = $parkir->akhir_berlaku;
        $this->tgl_dikeluarkan = $parkir->tgl_dikeluarkan;
        $this->keterangan = $parkir->keterangan;

        $this->resetValidation();
        $this->dispatch('open-modal', name: 'editBerlanggananModal');
    }
    ...
    public function update()
    {
        ...
        $parkir = ParkirBerlangganan::findOrFail($this->parkir_id);
        $parkir->update([
            'nomor' => $this->nomor,
            'jenis' => $this->jenis,
            'status' => $this->status,
            'nama_pemilik' => $this->nama,
            'nama' => $this->nama, // Let's populate both
            'no_pol' => $this->no_pol,
            'alamat' => $this->alamat,
            'jumlah' => $this->jumlah,
        ...
        $this->dispatch('close-modal', name: 'editBerlanggananModal');
...
<div>
    <div class="modal fade" id="editBerlanggananModal" tabindex="-1" aria-labelledby="editBerlanggananModalLabel" aria-hidden="true" wire:ignore.self>
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="editBerlanggananModalLabel">Edit Parkir Berlangganan</h5>
                ...
                <form wire:submit.prevent="update">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit" class="btn btn-primary">
                            <i class="ti ti-device-floppy me-1"></i> Simpan Pebaikan
                        </button>
                    </div>
                </form>
            </div>
